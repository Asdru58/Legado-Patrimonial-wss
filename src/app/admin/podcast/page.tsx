...
// Server Component: inventario de episodios (CRUD — Read)
// =========================================================

import Link from 'next/link'
import { getAllEpisodiosAdmin } from '@/lib/services/podcast'
...
export default async function AdminPodcastPage() {
  const episodios = await getAllEpisodiosAdmin()
  const hasData = episodios.length > 0
    ...
    <div>
      ...
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
        <div>
          ...
          >
            {hasData
              ? `${episodios.length} episodio${episodios.length !== 1 ? 's' : ''} registrado${episodios.length !== 1 ? 's' : ''}.`
              : 'Gestión de episodios del podcast.'}
          </p>
        </div>

        <Link
          href="/admin/podcast/nuevo"
          className="
      ...
      {!hasData && (
        ...
        >
          ...
          <p
            ...
            "
          >
            Aún no se han creado episodios. Comienza añadiendo el primero.
          </p>
        </div>
      )}
        ...
        <div className="border border-white/[0.06] rounded-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {episodios.map((ep) => {
                  const status = ep.published ? statusConfig.published : statusConfig.draft
                  const seasonEp = ep.numero_episodio
                    ? `T${ep.temporada} · E${String(ep.numero_episodio).padStart(2, '0')}`
                    : `T${ep.temporada}`
                  return (
                    <tr
                      key={ep.id}
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-right">
                        <Link
                          href={`/admin/podcast/${ep.id}/editar`}
                          className="
