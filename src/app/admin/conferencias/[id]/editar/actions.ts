...
// src/app/admin/conferencias/[id]/editar/actions.ts
// Server Action: actualizar conferencia + taxonomía via RPC atómica
// =========================================================
...
export interface EditarConferenciaState {
  success: boolean
  error: string | null
...
export async function editarConferencia(
  _prevState: EditarConferenciaState | null,
  formData: FormData
): Promise<EditarConferenciaState> {
  ...
  if (!id || !UUID_REGEX.test(id)) {
    return { success: false, error: 'Identificador de conferencia inválido.', fieldErrors: {} }
  }
  ...
  const tematica_ids = Array.from(
    new Set(raw_tematica_ids.filter((tid) => UUID_REGEX.test(tid)))
  )
  ...
  const { data: updatedId, error: rpcError } = await supabase.rpc(
    'editar_conferencia_con_taxonomia',
    {
      p_id: id,
      p_titulo: titulo,
      p_slug: slug,
  ...
  if (rpcError) {
    console.error('editarConferencia rpcError:', rpcError)

    if (rpcError.code === '23505') {
      return {
        success: false,
        error: null,
        fieldErrors: { slug: 'Este slug ya está en uso por otra conferencia.' },
      }
    }

    if (rpcError.message?.includes('Conferencia no encontrada')) {
      return {
        success: false,
        error: 'La conferencia no existe o no pudo ser actualizada.',
        fieldErrors: {},
      }
    }
    ...
    return {
      success: false,
      error: 'No fue posible actualizar la conferencia.',
      fieldErrors: {},
    }
  }
