...
| Post-ejecución | Inmediatamente tras la confirmación de la purga, el Administrador (Abg. Asdrúbal Lira) procederá a rotar la contraseña de la base de datos desde el dashboard de Supabase (Settings → Database → Database Password → Reset), y Antigravity purgará la variable `DATABASE_URL` de su entorno local. Esto invalida técnicamente cualquier acceso futuro, convirtiendo el veto en un bloqueo de infraestructura, no solo una regla de comportamiento |
